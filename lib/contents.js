...
const md = new MarkdownIt({
  ...
  speakerdeck(str) {
    ...
    return `<div class="embed-speakerdeck" style="position:relative;padding-bottom:${100/ratio}%;height:0;overflow:hidden"><iframe src="https://speakerdeck.com/player/${escapedKey}" style="position:absolute;top:0;left:0;width:100%;height:100%;border:0" scrolling="no" allowfullscreen allow="encrypted-media" loading="lazy"></iframe></div>`;
  },
  youtube(key) {
    if (!key?.match(/^[a-zA-Z0-9_-]+$/)) {
...
export function getSortedContentsData(subDirectory, locale) {
  const fileNames = fs.readdirSync(path.join(contentsDirectory, subDirectory)).filter(f => RegExp(`.${locale}.md$`).test(f))
...
export async function getContentData(subDirectory, id, locale) {
  ...
  contentHtml = contentHtml.replace(RegExp('<p><img src=\"([^\"]+)\" alt=\"([^\"]+)\"></p>', 'g'), '<figure><img src=\"$1\" alt=\"$2\"><figcaption>$2</figcaption></figure>')

  const al = (locale === 'en') ? 'ja' : 'en'
  matterResult.data.isTranslated = fs.existsSync(path.join(contentsDirectory, subDirectory, id.replace(`.${locale}.`, `.${al}.`))) ? 'true' : 'false'
