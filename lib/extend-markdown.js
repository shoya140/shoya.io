...
export function insertReferences(str) {
  const outputs = str.split(' ').map((key) => {
    ...
    outputItem += `. <b>&#147;<a href="#' + key + '" class="anchor-title">${entry.TITLE}</a>&#148;</b>. `
    ...
    outputItem = outputItem.replace('{\\ss}', '&szlig;')
    outputItem = outputItem.replace('{\\"O}', '&Ouml;')
    outputItem = outputItem.replace('{\\&}', '&amp;')
    outputItem = outputItem.replace('--', '&ndash;')
    outputItem = outputItem.replace('Shoya Ishimaru', '<u>Shoya Ishimaru</u>')

    return `<li><p class="reference"><span class="anchor-position" id="' + key + '"></span>${outputItem}</p></li>`
  })
